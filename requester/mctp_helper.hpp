...
class MctpMessageTransport : public NonCopyable
{
  public:
     ...
     */
    libspdm_return_t decode([[maybe_unused]] uint8_t eid,
                            std::vector<uint8_t>& buf, void** message,
                            size_t* message_size)
    {
        const size_t headerSize = sizeof(HeaderType);
...
class MctpIoClass : public IOClass
{
  ...
  private:
    ...
    inline libspdm_return_t writeToSocket(const std::vector<uint8_t>& buf)
    {
        if (!isSocketOpen())
        {
            lg2::error("Socket is not open (fd={FD})", "FD", socketFd);
            return LIBSPDM_STATUS_SEND_FAIL;
        }

        uint8_t eid = (buf.size() > 1) ? buf[1] : 0;
        ...
        ssize_t bytesSent =
            sendto(socketFd, buf.data() + 3, buf.size() - 3, 0,
                   reinterpret_cast<struct sockaddr*>(&addr), sizeof(addr));
        if (bytesSent < 0)
        {
            int err = errno;
            lg2::error(
                "Failed to send data through MCTP socket (fd={FD}), errno={ERRNO} ({ERRMSG})",
                "FD", socketFd, "ERRNO", err, "ERRMSG", std::strerror(err));
            return LIBSPDM_STATUS_SEND_FAIL;
        }
        return LIBSPDM_STATUS_SUCCESS;
    }
    ...
    inline libspdm_return_t readFromSocket(std::vector<uint8_t>& buf)
    {
        ...
        // Peek to get the actual message length
        ssize_t bufLen = recv(socketFd, nullptr, 0, MSG_PEEK | MSG_TRUNC);
        if (bufLen < 0)
        {
            lg2::error("Error peeking buffer length: {ERRNO}", "ERRNO", errno);
            return LIBSPDM_STATUS_RECEIVE_FAIL;
        }

        buf.resize(bufLen);
        ssize_t ret =
            recvfrom(socketFd, buf.data(), buf.size(), MSG_TRUNC,
        ...
        if (ret == -1)
        {
            lg2::error("Receive error: {ERRNO}", "ERRNO", errno);
            return LIBSPDM_STATUS_RECEIVE_FAIL;
        }
        return LIBSPDM_STATUS_SUCCESS;
    }
